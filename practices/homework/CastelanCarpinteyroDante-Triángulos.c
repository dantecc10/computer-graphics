...
void drawTeacher(int x, int y, int min) {
	// Supongamos que el m�nimo son 10 px
	...
	// Brazo 4
	fillRectangle(x + (min * 60), x + (min * 75), y - (min * 80), y - (min * 50), 0, .2, .6, 1);
...
void pinta(void){
	...
	// Fin de relleno de pizarr�n
	tableChair(100, 460);
	tableChair(400, 460);
	tableChair(700, 460);
	tableChair(1000, 460);
	tableChair(1300, 460);
	// Termina sal�n de clases
	glEnd();
	glFlush();
}
	...
	int main(int argc, char ** argv) {
		...
		glutInit(&argc, argv);
		glutCreateWindow("Dante Castel�n Carpinteyro - Graficaci�n");
